...
function drop(ev) {
    console.log(+ev.srcElement.id.split("")[0])
    const imgid= ev.dataTransfer.getData("imgid");
    const imagem = document.querySelector(`#${imgid}`)
    
    if ((+ev.srcElement.id.split("")[0]%2 == 0 && +ev.srcElement.id.split("")[1]%2 == 0) ||  (+ev.srcElement.id.split("")[0]%2 != 0 && +ev.srcElement.id.split("")[1]%2 != 0)) {
